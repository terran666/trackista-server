...
/**
 * bullFlagDetector.js (Phase 2 — disabled by default)
 * LONG: sharp move up (pole) → consolidation channel below resistance.
...
function detect({ candles, extremes, config, effectiveTolPct, debugLog }) {
  if (!config.enabledPatterns?.BULL_FLAG) return [];
  ...
  for (const level of resistanceLevels) {
    ...
    // Detect pole: first ~30% of bars should have a strong upward move
    ...
    const poleMove   = polePart.length > 1 ? (polePart[polePart.length - 1].close - polePart[0].open) / polePart[0].open * 100 : 0;
    ...
    const touches = findTouches(flagPart, levelPrice, tolAbs, 'resistance');
    ...
    candidates.push({
      ...
      direction: 'LONG',
      levelPrice,
      level: { price: levelPrice, side: 'resistance', source: 'sharp-extremes', startTimestamp: level.points?.[0]?.timestamp ?? null },
      pattern: {
        touches: touches.length, barsInside,
