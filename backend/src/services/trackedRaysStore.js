  ...
  try {
    ...
    if (fs.existsSync(bak)) {
      try {
        ...
        console.error('[tracked-rays-store] main file corrupted — restored from .bak');
        writeStore(store);
        return store;
      } catch (_) {}
    }
    console.error('[tracked-rays-store] data file corrupted, reinitializing:', err.message);
    const empty = { nextId: 1, rays: [], fingerprints: {} };
    writeStore(empty);
    return empty;
  }
}
